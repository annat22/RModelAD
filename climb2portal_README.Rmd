...
3. Set-up Synapse Login
Enter your synapse credential in the code lines below and run it in the consule. They will be stored in your system so it will not be necessary to enter in future sessions.
```{r, eval=FALSE}
suppressPackageStartupMessages({
  library(synapser)})
synLogin(email="yourEmail", password="yourPassword", rememberMe = TRUE)
```

4. Optional: download and install RStudio https://www.rstudio.com/products/rstudio/download/#download
...
You can it step-by-step, or you can follow steps 1-5 and then run step 11. This will create all metadata files and manifest file but will not upload anything yet.
...
To generate only individual metadata file, use ids_key.csv to provide animal names (individualID), leave all other columns empty, and skip steps 6, 7, and 9 below.
      ...
      * ids_key.csv ([see example here](https://github.com/annat22/RModelAD/blob/master/ids_key.csv))  
      ...
      * files_key.csv ([see example here](https://github.com/annat22/RModelAD/blob/master/files_key.csv))  
...
7. Generate and export manifest file from ids_key.csv  
```{r}
source("https://raw.github.com/annat22/RModelAD/master/gen_manifest.R")
idkey <- read_csv("ids_key.csv")
manifest <- gen_manifest(idkey)
write_tsv(manifest, file=paste0("manifest_",study,".tsv"))
```

8. Generate and export individual metadata from ids_key.csv  
...
9. Generate and export biospecimen metadata file from ids_key.csv  
...
After completing and validating the files, you can use the following lines to upload everything.
```{r, eval=FALSE}
suppressPackageStartupMessages({
  library(synapser)})
synLogin(silent=TRUE)
syncToSynapse(paste0("manifest_",study,".tsv"))

```

11. Run all.  
It is possible to run steps 6-9 continuously, using the following line of code. 
